...
/// <summary>
/// CSV에서 받아온 보상 데이터를 담아둘 클래스. <br/>
/// 이중 딕셔너리로 작업 하다가 이 방식이 더 보기에도, 사용하기에도 좋아보여 수정.
/// </summary>
...
/// 때문에 시스템 자체를 바꾸지 않는다면, 스테이지 클리어 시 단계를 올리기 전에 기존 방치 보상을 수령시키고 해당 시점을 기존 시간으로 재설정 후 스테이지를 올리는 방법으로 진행하려 함.
/// <br/> 아... 보상 받고 남는 시간 그냥 버려버리고싶다... 어떻게 보관해줄까...
/// </summary>
public class IdleTest : MonoBehaviour
{
    ...
    [SerializeField] private float maxIdleTime = 600.0f;    // 최대 방치 시간 (단위: 초)
    ...
    public void ClickIdleRewardButton()
    {
        ...
        ***********/
        // 버튼을 눌렀을 때 보상 딕셔너리 값 중 0보다 큰 값이 있는지 확인.
        if (rewards.Any(reward => reward.Value > 0))
        {
            ...
            ***********/
            ...
            // 스테이지별 보상 테이블에서 현재 스테이지 값을 통해 알맞은 보상 테이블 반환
            if (stageRewards.TryGetValue(testClearStageNum, out StageRewardData stageData))
            {
                ...
                KeyValuePair<,> 방식은 딕셔너리 반환에 좋다.
                ***********/
                // 딕셔너리를 순회하며 반환 받은 보상 테이블의 정보를 key: 재화 종류, value: 보상량 형태로 사용
                foreach (KeyValuePair<CurrencyType, float> rewardInfo in stageData.Rewards)
                {
                    CurrencyType type = rewardInfo.Key;    // 재화 종류 저장
                    decimal rewardAmount = (decimal)rewardInfo.Value;    // 재화 별 보상량 저장, decimal로 변환
                    decimal leftReward = leftRewards.ContainsKey(type) ? leftRewards[type] : 0m;    // 재화 별 이전 수령 후 남은 시간량 계산

                    decimal finalAmount = (rewardTime * rewardAmount) + leftReward;    // 남은 시간도 포함한 최종 보상량 계산
                    int finalReward = (int)finalAmount;    // 최종 보상량을 int로 형변환. 안 하고 결과값에 그냥 (int)해도 됨.

                    leftRewards[type] = finalAmount - finalReward;    // 소수점 값 (보상 정산에 들어가지 못 하는 시간) 남은 시간 보관용 딕셔너리에 저장
                    // 최종 보상 지급량이 1개 이상인 경우 재화 지급
                    if (finalReward > 0)
                    {
    ...
    private Dictionary<CurrencyType, int> CalculateRewards()
    {
        ...
        if (stageRewards.TryGetValue(testClearStageNum, out StageRewardData stageData))
        {
            ...
            foreach (KeyValuePair<CurrencyType, float> rewardInfo in stageData.Rewards)
            {
                ...
                decimal reward = (decimal)rewardInfo.Value;    // 보상량 형변환 후 저장

                decimal leftReward = leftRewards.ContainsKey(type) ? leftRewards[type] : 0m;    // 이전 보상 수령 후 남은 시간이 있다면 반환

                decimal finalReward = (rewardTime * reward) + leftReward;    // 기본 보상량과 남았던 보상량 합산

                calculateRewards[type] = (int)finalReward;    // 최종 보상 int형으로 반환
            }
        }
